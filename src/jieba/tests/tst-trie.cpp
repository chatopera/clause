...
static const char* const DICT_FILE = "../../../../var/test/jieba/testdata/extra_dict/jieba.dict.small.utf8";

TEST(JiebaTest, TrieTestEmpty) {
  vector<Unicode> keys;
  vector<const DictUnit*> values;
  Trie trie(keys, values);
}

TEST(JiebaTest, TrieTestConstruct) {
  vector<Unicode> keys;
  vector<const DictUnit*> values;
...
TEST(JiebaTest, DictTrieTestNewAndDelete) {
  DictTrie * trie;
  trie = new DictTrie(DICT_FILE);
  delete trie;
}

TEST(JiebaTest, DictTrieTest1) {
  string s1, s2;
...
TEST(JiebaTest, DictTrieTestUserDict) {
  DictTrie trie(DICT_FILE, "../../../../var/test/jieba/testdata/userdict.utf8");
  string word = "云计算";
...
TEST(JiebaTest, UserDictWithMaxWeight) {
  DictTrie trie(DICT_FILE, "../../../../var/test/jieba/testdata/userdict.utf8", DictTrie::WordWeightMax);
  string word = "云计算";
  cppjieba::RuneStrArray unicode;
  ASSERT_TRUE(DecodeRunesInString(word, unicode));
  const DictUnit * unit = trie.Find(unicode.begin(), unicode.end());
...
TEST(JiebaTest, DictTrieTestDag) {
  DictTrie trie(DICT_FILE, "../../../../var/test/jieba/testdata/userdict.utf8");
